...
public class ProposalService : IProposalService
{
        ...
        Guid id, Guid userId, CreateProposalRequest req, CancellationToken ct = default)
    {
        ...
        p.Discount      = ComputeDiscount(updatedVehicles, req.Discount);
        await _uow.SaveChangesAsync(ct);
    ...
    private static Proposal BuildProposal(Guid userId, Guid clientId, CreateProposalRequest req)
    {
        ...
        return new()
        {
            ...
            Discount              = ComputeDiscount(vehicleList, req.Discount),
    ...
    /// <summary>
    /// Mirrors ComputeProposalValue: when any vehicle has its own Discount, sum those as the
    /// proposal-level discount instead of relying on the separate manual field — the "Nova
    /// Proposta" flow only collects per-vehicle discount, it has no standalone discount input.
    /// </summary>
    private static decimal? ComputeDiscount(
        IList<ProposalVehicleRequest>? vehicles, decimal? manualDiscount)
    {
        if (vehicles is not null && vehicles.Any(v => v.Discount.HasValue))
            return vehicles.Sum(v => v.Discount ?? 0m);
